...
def comparer():
    ...
    if request.method == 'POST':
        rawtext = request.form['rawtext']
        final_reading_time = readingTime(rawtext)
        final_summary_space = text_summarizer(rawtext)
        summary_reading_time = readingTime(final_summary_space)
        # Gensim Summarizer
        ...
        final_time = end - start
    return render_template('compare_summary.html', ctext=rawtext, final_summary_spacy=final_summary_space,
